...
blog = {"dir":environ.get('BLOG_CONTENT_DIR'),"imagepath":environ.get('BLOG_IMAGE_PATH')}
...
def create_thumbnail(filename):
    ...
    newThumb = True
    try:
        contents = repo.get_contents(f"static{blog['imagepath']}/thumb/{filename}", ref="main")
        if contents:
    ...
    if newThumb:
        ...
        for filepath in filelist:
            ...
            try:
                contents = repo.get_contents(f"static{blog['imagepath']}/thumb/{fn}", ref="main")
        ...
        thispic = f.read()
        try:
            repo.create_file(f"static{blog['imagepath']}/thumb/{filename}",'app',thispic,branch="main")
        except:
            None
...
def github_save_file(path):
    ...
    if not exists(photo_path):
        try:
            ...
            repo = g.get_repo(repository)
            content_encoded = repo.get_contents(urllib.parse.quote(g_path), ref='main').content
...
def post_new_file():
    ...
    for k,v in request.form.items():
        ...
        if  k != 'filecontent' and k.find('photo_') == -1 and k != 'path' and k.find('__') == -1 :
...
#
    ...
    if request.files['__new_photo'].filename > '':
        photoset=process_new_photo(request.files['__new_photo'],request.form['__new_photo__caption'],file,photoset)
    p=json.dumps(photoset)
    fc = fc + "photoset: " + p + "\n"
    fc = fc + "---\n"
    fc = fc + filecontent
